...
export async function login(req, res, next) {
  try {
    ...
    const valid = user.verifyPassword ? await user.verifyPassword(password) : await bcrypt.compare(password, user.password || user.passwordHash || '');
    ...
    const safeUser = { id: user._id.toString(), email: user.email, name: user.name || '', role: user.role || 'user' };
